...
def is_installed(*packages):
    ...
    return tuple(ifilterfalse(partial(is_, True), imap(
        lambda package: (run("rpm -q --queryformat '%{VERSION}' " + '{}'.format(package.name),
                             warn_only=True).startswith(package.version)
                         if isinstance(package, Package)
                         else run('rpm -q {package}'.format(package=package),
...
def yum_depends(*packages):
    ...
    return sudo('yum install -y {packages}'.format(
...
def get_pretty_name():
    """ E.g.:  """
    name = run('cat /etc/*elease | grep release | uniq')  # `CentOS Linux release 7.4.1708 (Core)`
    return '{dist} {version}'.format(dist=name.partition(' ')[0],
